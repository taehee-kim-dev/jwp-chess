...
- [ ] 게임 진행
    - [ ] 이동 요청 유효성 검사
        - [ ] 이동 요청에 같이 들어온 쿠키에서 값을 꺼내 검사한다.
            - [ ] encryptedPassword key가 있는지 확인한다.
                - [ ] 없다면 "쿠키에 encryptedPassword key가 없습니다." 의 메시지로 예외를 던진다.
            - [ ] encryptedPassword 값이 DB에 저장되어 있는 현재 차례 플레이어의 hash 암호화 된 비밀번호와 일치하는지 확인한다.
                - [ ] 일치하지 않다면, "쿠키로 전달된 암호화 된 encryptedPassword 값이 일치하지 않습니다." 의 메시지로 예외를 던진다.
